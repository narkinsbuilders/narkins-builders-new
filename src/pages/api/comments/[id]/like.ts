...
export default async function handler(
  ...
  try {
    ...
    const userFingerprint = Buffer.from(`${userIp}-${userAgent}`).toString('base64').slice(0, 32);
    
    // Toggle like
    const liked = await LikeQueries.toggleLike(commentId, userIp, userFingerprint);
