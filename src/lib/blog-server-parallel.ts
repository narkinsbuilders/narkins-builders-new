...
export function getAllPostsServer(): BlogPost[] {
 ...
 if (fs.existsSync(indexPath)) {
  try {
   ...
   cacheTimestamp = now
   console.log(`[BLOG] Loaded ${postsCache.length} posts from parallel cache`)
   return postsCache
  } catch (error) {
   console.warn('Failed to load parallel blog cache, falling back to file system')
  }
 }
...
export function getPostBySlugServer(slug: string): BlogPost | null {
 ...
 if (fs.existsSync(cacheFilePath)) {
  try {
   ...
   console.warn(`Failed to load cached post ${slug}, falling back to file system`)
  }
 }
