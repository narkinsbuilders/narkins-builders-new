...
export function filterAndSortPosts(posts: BlogPost[], filters: BlogFilters): BlogPost[] {
 let filteredPosts = [...posts];

 // Filter by content type
 if (filters.contentType !== 'all') {
  filteredPosts = filteredPosts.filter(post => 
   categorizePost(post) === filters.contentType
  );
 }
 ...
 if (filters.readTime !== 'all') {
  filteredPosts = filteredPosts.filter(post => 
   getReadTimeCategory(post.readTime) === filters.readTime
  );
 }
