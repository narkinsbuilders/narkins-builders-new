...
export default function InvestmentFunnel({ data, title }: InvestmentFunnelProps) {
  ...
  const validateData = (inputData: any): typeof defaultData => {
    ...
    return inputData.map(item => ({
      ...
      percentage: typeof item?.percentage === 'number' && !isNaN(item.percentage) ? Math.max(0, Math.min(100, item.percentage)) : undefined
    }));
  };
