...
interface EChartsPictorialBarChartProps {
  ...
  title: string
  subtitle?: string
  symbolType?: 'rect' | 'roundRect' | 'triangle' | 'diamond' | 'pin' | 'arrow' | 'circle' | 'building' | 'house' | 'custom'
  color?: string | string[]
  height?: number
...
export default function EChartsPictorialBarChart({ 
  ...
  const customSymbols = {
    building: 'path://M0,10 L0,0 L10,0 L10,10 Z M2,8 L3,8 L3,6 L2,6 Z M5,8 L6,8 L6,6 L5,6 Z M7,8 L8,8 L8,6 L7,6 Z M2,5 L3,5 L3,3 L2,3 Z M5,5 L6,5 L6,3 L5,3 Z M7,5 L8,5 L8,3 L7,3 Z',
    house: 'path://M5,0 L0,5 L0,10 L2,10 L2,7 L8,7 L8,10 L10,10 L10,5 Z M3,8 L3,9 L7,9 L7,8 Z',
    custom: 'path://M0,0 L5,0 Q10,0 10,5 L10,10 Q10,15 5,15 L0,15 Q-5,15 -5,10 L-5,5 Q-5,0 0,0 Z'
  };
  ...
  const seriesData = data.map((item, index) => {
    const itemColor = item.color || (Array.isArray(color) ? color[index % color.length] : color);
    
    return {
      name: item.name,
      value: item.value,
      symbol: item.symbol || customSymbols[symbolType as keyof typeof customSymbols] || symbolType,
      symbolSize: Array.isArray(symbolSize) ? symbolSize : [symbolSize, symbolSize * 0.2],
      itemStyle: {
