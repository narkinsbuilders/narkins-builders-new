           ...
           <Tab.Panel key={category.name} className="space-y-10 px-4 pb-8 pt-10">
            <div className="grid grid-cols-2 gap-x-4 gap-y-4">
             {category.featured.map((item) => (
              <div key={item.name} className="group relative text-sm">
               ...
               <Link href={item.href} className="mt-6 text-black block ">
            ...
            {category.sections.map((section) => (
             <div key={section.name}>
              <p id={`${category.id}-${section.id}-heading-mobile`} className=" text-black">
               {section.name}
              </p>
              <ul
               role="list"
        ...
        <div className="space-y-6 border-t border-gray-200 px-4 py-6">
         {navigation.pages.map((page) => (
          <div key={page.name} className="flow-root">
           <Link href={page.href} className="-m-2 block p-2 text-black ">
            {page.name}
           </Link>
          </div>
         ))}
                   ...
                   <div className="grid grid-cols-2 gap-x-3 gap-y-3">
                    {category.featured.map((item) => (
                     <div key={item.name} className="group relative text-sm">
                      ...
                      <Link href={item.href} className="mt-2 block text-gray-900 text-sm">
                       <span className="absolute inset-0" aria-hidden="true" />
                       {item.name}
                      </Link>
                     </div>
